...
def get_currency_symbol(country_name):
    country_data = [c for c in countries if c.get("name").lower() == country_name]
    print(country_data)
...
def get_all_countries_border_count():
    ...
    border_count = {c.get("name") :len(c.get("borders",[0])) for c in countries}
...
def get_area(dictionary):
    ...
    return dictionary.get("area",0)

longest_cntry = max(countries,key=get_area)
